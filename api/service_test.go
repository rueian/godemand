	...
	Describe("RequestResource", func() {
		...
		Context("call controller's FindResource", func() {
			...
			Context("err from controller", func() {
				BeforeEach(func() {
					controller.EXPECT().FindResource(p, types.Merge(pcfg.Params, client.PoolConfig)).Return(types.Resource{}, errors.New("any"))
				})
				It("got err", func() {
			...
			Context("one of resources from controller", func() {
				BeforeEach(func() {
					controller.EXPECT().FindResource(p, types.Merge(pcfg.Params, client.PoolConfig)).Return(types.Resource{ID: "a", PoolID: poolID}, nil)
				})
				It("got no err", func() {
			...
			Context("new resources from controller", func() {
				BeforeEach(func() {
					controller.EXPECT().FindResource(p, types.Merge(pcfg.Params, client.PoolConfig)).Return(types.Resource{ID: "b", PoolID: poolID}, nil)
				})
				It("got no err", func() {
					Expect(err).NotTo(HaveOccurred())
				})
